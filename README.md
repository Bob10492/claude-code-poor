...
> 当前 README 聚焦本仓库在 **本地可观测、实验评测、反馈闭环** 方向上的演进，不再复述原 CCB 的完整功能清单。
...
它不是单纯的日志系统，也不是只给人看的 dashboard。它的目标是把一次真实的用户动作拆成可以追踪、可以复盘、可以比较、可以生成下一步建议的工程事实链：
...
1. **发生了什么？** 这次动作到底展开成了哪些 query、turn、tool、subagent 和 snapshot？
2. **代价是多少？** token、缓存、延迟、工具调用、子链路放大分别消耗在哪里？
3. **下一步该怎么改？** baseline 和 candidate 的差异是否真实、稳定、可解释？评测结果能否生成可审批的下一步方案？
...
flowchart TB
  ...
  Reports --> V11[V1.1 Deep Action Analysis<br/>single user_action_id timeline / causality / flowchart]
...
| **V1.1** | 单个 `user_action_id` 深度解析层 | 如何针对一个 action id 还原完整时序、因果、分支、延迟、输出与复杂流程图？ | action 深度报告、时序对齐、因果分析、复杂 Mermaid 流程图 |
...
> 注：这里的 **V1.1** 指的是 `E:\claude-code\ObservrityTask\10-系统版本\v1` 下围绕“深度解析一个 `user_action_id`”形成的专题目录与分析方法。它不是泛称的 Bridge 层，而是把 V1 的事实表、事件、snapshot 和 report 能力收敛到单个 action 的完整复盘上。
...
## V1.1：深度解析一个 `user_action_id`

V1.1 是 V1 目录中最重要的“单次动作深挖”能力层，相关材料位于：

```text
E:\claude-code\ObservrityTask\10-系统版本\v1
```

它的目标不是再做一个抽象总览，而是拿一个明确的 `user_action_id`，把这次动作从“用户提交 query”到“主线程、子 agent、工具调用、snapshot、输出、延迟、成本、分叉原因”完整还原出来。

```mermaid
flowchart TD
  UA["user_action_id<br/>单次用户动作"] --> Q0["主线程 query"]
  Q0 --> T1["turn-1<br/>request / response / state"]
  T1 --> TU["tool_use / tool_result"]
  T1 --> S1["spawn subagent<br/>记录触发原因"]
  S1 --> Q1["subagent query"]
  Q1 --> ST["subagent turns"]
  Q0 --> Cost["主线程 token / latency"]
  Q1 --> SubCost["子链路 token / latency"]
  TU --> Evidence["证据对齐<br/>events + snapshots"]
  ST --> Evidence
  Cost --> Evidence
  SubCost --> Evidence
  Evidence --> Report["深度解析报告"]
  Report --> Mermaid["复杂 Mermaid 流程图"]
```

V1.1 重点回答这些问题：

- 这个 `user_action_id` 下到底有几条 query？
- 哪条是主线程，哪些是 `session_memory` / `extract_memories` 等子链路？
- 每个 turn 大概做了什么，输入、输出、工具调用如何变化？
- 哪些 snapshot 是关键证据，request / response / state 如何对齐？
- subagent 是在什么时刻、因为何种 trigger 被拉起？
- 主线程和子线程分别消耗了多少 token，延迟大概在哪里？
- 最终能否把整个过程画成比普通 DAG 更细的 Mermaid 流程图？
...
如果 V1 是“把事实记录下来”，那么 V1.1 就是“围绕一个 action id，把事实读懂、讲清、画出来”。
...
```mermaid
flowchart TD
  A[先读本 README] --> B[ObservrityTask/README.md]
  B --> C[V1 深度研究报告]
  C --> D[V1.1 单 action 深度解析材料]
...
4. 进入 V1.1 思路：围绕一个 `user_action_id` 对齐 request / response / state / tool / subagent
5. 如果要进入评测，再把 `user_action_id` 绑定到 V2 run

### 如果你要判断一个 candidate 是否值得继续
...
6. 如果是 V2.4+，必须看 `long_context_summary`
7. 如果是 V2.5+，最后看 `approval card`
...
sequenceDiagram
  ...
  participant A as V1.1 Analysis
  ...
  P->>A: 选定 user_action_id
  A->>U: 深度报告 + 复杂 Mermaid + 因果说明
```

### 工作流 B：比较 baseline 与 candidate
...
### 5. V1.1 必须围绕具体 action id 展开

V1.1 的价值不是再写一份总览，而是从一个具体 `user_action_id` 出发，把证据链读细、读准、读成流程图。
...
V1 是事实观测系统，负责把事件、snapshot 和 DuckDB facts 建起来。V1.1 是围绕一个明确 `user_action_id` 的深度解析方法，把主线程、子线程、工具调用、snapshot、时序、因果、成本和流程图组织成一份可读分析。

### Q: V1 和 V2 的区别是什么？

V1 是事实观测系统，回答“发生了什么”。V2 是评测系统，回答“baseline 和 candidate 的差异是否有证据支撑”。
...
当前仓库在此基础上加入并整理了本地可观测、V1.1 单 action 深度解析、V2 评测、长上下文专项与反馈闭环相关内容。
